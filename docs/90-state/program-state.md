...
**MI-005 Resumable Run Cutover is complete with unconditional PASS and its 47-path Nestfolio Level 5 evidence delta is published; the SE-001 selected effort implementation and evidence are published; MI-006 — Evidence-Bound Completion and Guard Coexistence executed 2026-07-18 and closed UNCONDITIONAL FAIL (`ENGINE_POLICY_MISMATCH`) fail-closed before any Nestfolio mutation; MI-006-R1 — Evidence-Bound Completion Existing-Work-Item Rebinding Correction executed 2026-07-18 and closed UNCONDITIONAL PASS — the program's first Evidence-bound Work completion, through the pinned engine unmodified, with the one target-routed effort advanced to Level 6 completed and its 52-path Nestfolio execution delta now published; the MI-007 — Governed Learning and Selective Current-Runtime Retirement contract is authored, owner-reviewed, and published, and MI-007 executed 2026-07-18 and closed UNCONDITIONAL PASS — the program's first governed Decision-and-Learning slice, converting the completed effort's run-mi006-r1 Evidence into five Observations, two Lessons, one durable Decision, one separate Change Proposal, and exactly ONE owner-applied additive project-binding rule through the pinned store API with the engine unmodified (recordLesson never invoked), with a no-auto-promotion proof and a six-mechanism retirement inventory that retires nothing; its 36-path Nestfolio execution delta is now published; the SD-001 — Sustained Dogfooding Bootstrap contract was authored, owner-reviewed, and published, and SD-001 executed 2026-07-18 and closed UNCONDITIONAL PASS — the frozen measurement protocol and the append-only dogfooding ledger were materialized exactly as contracted, the owner's live verbatim period-start confirmation ("vai") was captured, and SD-001 performed NO store write; its 2-path Nestfolio execution delta was published as SD-001-PUB at Nestfolio `bd0b2fcc2d56fecbcc45c05c80cfe135605b44d3`, and the sustained-dogfooding period began at that commit's committer UTC `2026-07-18T21:39:42.000Z`; fourteen immutable results now stand; the period runs under the standing period rules with the pinned engine frozen, and its verdict (working name SD-002) remains blocked pending separate contracting until the minimum period is satisfiable.**
...
**On 2026-07-18, under DR-0024, MI-006 — Evidence-Bound Completion and Guard Coexistence was executed from the published contract revision `6a94e73c864627c3c33a27f3a891005df22984df` and closed UNCONDITIONAL FAIL with primary diagnostic `ENGINE_POLICY_MISMATCH`, fail-closed before any Nestfolio mutation; Nestfolio is byte-identical and clean at `eafa09b81a600bd58fe8f4639298a182039d21f5` (zero-path execution delta).** Session gates verified all three repositories clean on main (continuity-lab HEAD at the pinned contract revision; Nestfolio HEAD at the bound MI-005-PUB revision); the engine-lock (20 `runtime/continuity/**` files plus `.claude/settings.json`, aggregate `7e31ff56a10ca6b1715b29f02132e66c80e321921b41dcb55329db2e0f320a38`), every bound Level 5/4/3/2 digest, the run-mi005 closed check, and the read-only route check (no active current-journal or target Run for the effort; `.git/journal` 89 files across 30 run directories; tree digest `ae1506534f72092ec0200473e36d6d0ee39a8ce6a89444a2d4bf64e4ebd01b30`) all verified, and the baseline suites returned Level 5 7/7, Level 4 23/23, Level 3 23/23, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68, and dashboard-bff unit 70/70 in the real working tree. The blocking condition is structural: the pinned engine keys exactly one Work Item artifact per `work_id`, and `completeRun`'s truthful backlog write-back forces `workItem.id` to equal the effort id `dashboard-bff-awaiting-confirmation-activity-gap`; that work-item artifact already exists at revision 2, status `in_progress`, committed at `eafa09b8` as an immutable MI-005 record carrying the dogfood criterion `mi005-keyed-effect-materialized` (not the three Level 3 criteria), because MI-005 executed its Level 5 dogfood Run using the effort's real work id. `selectWork(dashboard-bff-awaiting-confirmation-activity-gap)` therefore returns `REVISION_CONFLICT` (expected_revision 0, actual_revision 2 — proven empirically in an isolated copy outside all three repositories, since removed with an absence proof), and reusing or advancing the existing Work Item via `startRun`/`completeRun` would mutate an immutable MI-005 record and validate the wrong criteria; no engine path exists without engine modification (prohibited) or an immutability violation (prohibited). An independent, owner-authorized secondary finding (machine-captured UTC `2026-07-18T11:38:34.000Z`) is recorded but is not the cause: criterion 2/3 deterministic Evidence references the dashboard-bff integration suite, but the awaiting-confirmation projection is asserted only in the unit suite and the integration suite targets real deployed AWS; the owner authorized reading criterion 2/3 Evidence as the unit suite plus a non-regression proof. No `run-mi006`, `continuity/level-6`, `continuity/evidence/mi-006`, Work Item binding, Guard binding, Assurance Plan, Evidence, completion, or backlog write-back was created. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, and MI-005 PASS remain ten separate immutable results, and MI-006 UNCONDITIONAL FAIL is a separate new immutable result (the eleventh); the SE-001-PUB and MI-005-PUB publication records are unchanged. Per `next_iteration_rule.on_fail`, the next operation is a newly bounded correction supported by the failure evidence — a program-level decision on enabling the effort's Level 6 completion (an engine revision that provides a fresh completion path for an effort whose work-item id was already used by a prior Run, or a re-scoped MI-006-R1 that explicitly authorizes advancing the existing `in_progress` Work Item under a bounded immutability exception and corrects the integration-suite Evidence wording). MI-007 and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, the MI-006-R1 — Evidence-Bound Completion Existing-Work-Item Rebinding Correction contract was authored, reviewed by the human program owner, and published on continuity-lab main.** It is the single newly bounded correction selected per the MI-006 `next_iteration_rule.on_fail`. The human program owner approved the reconciled option (b) design: no engine code. The authoring session verified against the pinned engine and the committed Nestfolio state at `eafa09b8` that the MI-006 recorded root cause holds exactly, that the naive option (b) reading — resuming and completing `run-mi005` — is NOT compliant (the run's immutable validation plan carries only the dogfood criterion `mi005-keyed-effect-materialized`; completing it would validate the wrong criteria), and that the contracted form executes end-to-end through the pinned engine unmodified, proven in an isolated scout copy outside all three repositories (since removed). The contract grants exactly ONE bounded immutability exception — the effort's existing Work Item artifact (immutable MI-005 record, revision-2 base SHA-256 `313af5905dbb3fda35dc9c193502df667812f0044c91f6c3b3691d136313aa60`) advances to completed exclusively via a contracted rebinding write (pinned store API, `expectedRevision 2`, criteria rebound to the three unchanged Level 3 criteria) followed by the pinned engine's `startRun`/`completeRun` for one new Run `run-mi006-r1` — plus a bounded formation exception (a new completion Scope `scope-mi006-r1-completion` and a new ready Working Set `ws-mi006-r1`, direct-authored through the pinned store API, the exact preparation mechanism MI-005 itself used and evidenced). Every other MI-005/VS-001/VS-001A record is pinned byte-identical by SHA-256; `run-mi005` is never reopened and, on PASS, becomes permanently non-resumable by staleness (no byte change — an accepted, documented consequence, not a repair). The criterion 2/3 deterministic Evidence is corrected to the dashboard-bff unit suite plus a byte-identity/non-regression proof of the integration-suite files (the owner-authorized MI-006 secondary finding). The contract also authorizes a bounded completion-aware correction of exactly four published suite assertions (Level 4 S7/S8/S9 and the Level 5 effort-source byte-identity test) that pin the current backlog bytes and were proven empirically — in the isolated copy, with the completion delta committed — to be the only assertions that fail after any truthful completion write-back (Level 1 14/14, Level 2 23/23, Level 3 23/23, and engine 9/9 stay green); MI-004-R1 is the precedent for correcting a self-invalidating published suite. Execution is NOT authorized by the authoring session: it requires a later fresh session launched under `sessions/MI-006-R1-evidence-bound-completion-existing-work-item-rebinding/prompt.md` with the exact published contract revision resolved, or an explicit same-session extension after publication. DR-0025 closure was performed (queue_position 33, queue_summary, indices regenerated, `--check` exit 0). MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, and MI-006 FAIL remain eleven separate immutable results, unrepaired and unrelabeled, and the SE-001-PUB and MI-005-PUB publication records are unchanged. MI-007 and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, MI-006-R1 — Evidence-Bound Completion Existing-Work-Item Rebinding Correction was executed from the published contract revision `80d70ac9bf9f1252474a9fbed414a0ef6ee57061` and closed UNCONDITIONAL PASS — the program's first Evidence-bound Work completion, executed through the pinned engine unmodified (no engine byte changed).** Every precondition verified fail-closed before mutation: session gates (three repositories clean on main; Nestfolio HEAD `eafa09b81a600bd58fe8f4639298a182039d21f5`), the engine-lock (20 `runtime/continuity/**` files plus `.claude/settings.json`, aggregate `7e31ff56…`), every bound Level 5/4/3/2 digest, the revision-2 Work Item base `313af590…` (envelope `011336b8…`), the full 16-record MI-005 byte-identity list, the run-mi005-closed check, the read-only route check (journal tree digest `57b5e86a…`, unchanged after execution), and the baseline suites (Level 5 7/7, Level 4 23/23, Level 3 23/23, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68, dashboard-bff unit 70/70). The effort's existing `in_progress` Work Item advanced from the immutable revision-2 MI-005 base to completed exclusively through the three contracted transitions — rev 3 rebinding write (pinned store API, `expectedRevision 2`; exactly the ten contracted field changes, `source.sha256` byte-equal `b656733…`), rev 4 engine `startRun`, rev 5 engine `completeRun` — under the single bounded immutability exception, with a bounded formation exception direct-authoring `scope-mi006-r1-completion` (rev 1; empty write paths/effect classes) and `ws-mi006-r1` (rev 1). The pinned engine's `startRun` formed the Assurance Plan from the three unchanged Level 3 criteria; `validateRun` produced three linked immutable Evidence artifacts spanning distinct deterministic, agent-review, and human-review modes (criterion 1 carries the owner's explicit human-review authorization "Autorizzo il finding", machine-captured UTC `2026-07-18T13:04:57.000Z`); exactly one dead-handler Guard was classified with full metadata (`nestfolio-dashboard-bff-no-dead-user-confirmation-requested-handler`) and evaluated true while the dashboard-bff suites remained validators; and `completeRun` atomically completed the Work Item (rev 5), recorded the final verified Checkpoint `run-mi006-r1-final-1`, linked the Evidence, and performed the truthful backlog write-back (`status shipped`, `closed 2026-07-18`; `b656733…` → `ea565c07…`, only tracking fields written). Every mandatory failure blocked with its exact typed diagnostic in isolated fixtures, and the distinct authorized Waiver path was demonstrated on a synthetic criterion; F-EXCEPTION-OVERREACH and F10–F12 were verified structurally. The four self-invalidating published assertions (Level 4 S7/S8/S9, Level 5 effort-source) received the bounded completion-aware correction and pass in both the pre-completion and post-completion states (totals unchanged 23 and 7; no product or engine code touched); post-completion the corrected Level 4 23/23 and Level 3 23/23 verified in an isolated committed validation copy (SE-001-R3/MI-005 mechanics) and every other required suite in the real tree. Every MI-005/VS-001/VS-001A byte-identity record is unchanged, the current journal tree is byte-identical, the audit prefix is preserved, and the Nestfolio execution delta is exactly the fixed create paths, the two formation writes, the frozen engine write-set, and the six permitted modified paths — 6 modified, 46 created, nothing staged, uncommitted and unstaged (manifest self-hash `840c12c0ae31e1884a4221372b873be5576bcd7097485d29bcb938e7d0b93fea`); its publication requires separate explicit authorization. As an accepted, documented consequence, `run-mi005` is now permanently non-resumable by staleness (its final Checkpoint pins work_item revision 2); no `run-mi005` byte changed and the MI-005 PASS is not repaired, relabeled, or reinterpreted. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, and MI-006 FAIL remain eleven separate immutable results, and MI-006-R1 UNCONDITIONAL PASS is a separate new immutable result (the twelfth); the SE-001-PUB and MI-005-PUB publication records are unchanged. MI-007 (Governed Learning and Selective Current-Runtime Retirement) remains the next contractable slice; MI-007 and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, the MI-006-R1 Nestfolio execution-delta publication was executed: session gates verified continuity-workspace and continuity-lab clean on main (continuity-lab HEAD `ea50615474748ee4f13be94a2cfec76c25e39e4f`) and Nestfolio HEAD `eafa09b81a600bd58fe8f4639298a182039d21f5` carrying exactly the recorded 52-path delta unstaged and byte-exact against the recorded SHA-256 manifest (manifest self-hash `840c12c0ae31e1884a4221372b873be5576bcd7097485d29bcb938e7d0b93fea` recomputed and verified, including the completed Work Item at revision 5 status completed and the backlog frontmatter status `shipped`, `closed 2026-07-18`); the 52 paths were staged and committed as one commit, exact subject "Publish MI-006-R1 evidence-bound completion Level 6 assurance", sole parent `eafa09b81a600bd58fe8f4639298a182039d21f5`, published at exact Nestfolio revision `6229bb010d76723aaec0385c923b157762ee512e`, and pushed to origin/main. A pre-existing repository ship-gate (`backlog-index-matches`: `docs/BACKLOG.md` stale relative to the already-frozen backlog frontmatter change in the recorded delta) was resolved through the repository's own journaled `RUNTIME_GATE_SKIP` mechanism, after clearing an unrelated stale journal writer lease for the same runId (dead pid, same machine confirmed by unbroken uptime, mismatched local mDNS hostname string only); the skip is journaled for ship-recheck adjudication and did not alter the 52-path manifest or its self-hash. At the published revision, Nestfolio is clean and Level 6, Level 5, Level 4, Level 3, Level 2, Level 1, engine, and backlog-next return 10/10, 7/7, 23/23, 23/23, 23/23, 14/14, 9/9, and 68/68 respectively, all directly in the real working tree once clean. This publication records repository state only: no completion, Run, Assurance, or Level 6 authority beyond what MI-006-R1 already recorded, and every prior immutable result and the MI-006-R1 execution record remain unchanged. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, and MI-006-R1 PASS remain twelve separate immutable results, unrepaired and unrelabeled. MI-007 (Governed Learning and Selective Current-Runtime Retirement) becomes the next contractable slice; MI-007 and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, the MI-007 — Governed Learning and Selective Current-Runtime Retirement contract was authored, reviewed by the human program owner, and published on continuity-lab main.** MI-007 is the Level 6 Decision-and-Learning slice completing the accepted operational loop: it converts Evidence-backed findings from the completed target-routed effort into governed target learning records through the pinned store API with the engine unmodified (`recordLesson` is excluded because it would advance the published run-mi006-r1 record). The owner approved the recommended full-loop design in the authoring session: five fixed Observations — four factual, sourced by digest from the run-mi006-r1 Evidence and the MI-006 closure, to be reviewed at execution (live, verbatim, machine-captured) to the contracted accepted/duplicate/deferred/rejected outcomes, plus one provenance-preserving historical import of the current LESSONS.md F-30 entry (digest-pinned, never promoted); two Lessons (accepted candidate; rejected-unsafe); one durable Decision and one separate Change Proposal; exactly ONE owner-applied additive project-binding record (the deterministic-evidence boundary rule under `continuity/bindings/nestfolio/decisions/`), created only after the recorded Decision and acceptance with an audit-order proof and reversible by removal; a no-auto-promotion digest proof over every Skill, Pack, engine, suite, binding, store, and lock byte; and a retirement inventory classifying the six migration-plan backward-edge mechanisms (retained/deferred/rejected for target-routed use; automatic check promotion prohibited for target-routed work) with NOTHING actually retired in this bound. The only pre-existing Nestfolio paths that change at execution are the append-only audit ledger and the derived index rebuild; every published record including every run-mi006-r1 and VS-001 learning artifact is pinned byte-identical. The store mechanics for the new decisions and change-proposals artifact kinds were proven empirically in an isolated scratch store outside all three repositories (since removed). Execution is NOT authorized by the authoring session: it requires either a later fresh session launched under `sessions/MI-007-governed-learning-and-selective-current-runtime-retirement/prompt.md` with the exact published contract revision resolved and the human program owner available in-session for the live review capture, or an explicit same-session extension after publication. DR-0025 closure was performed (queue_position 34, queue_summary, the generator's permanent blocked tail reduced from "MI-007 — blocked" to the single "broader implementation and unrelated migration — blocked" entry, indices regenerated, `--check` exit 0). MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, and MI-006-R1 PASS remain twelve separate immutable results, unrepaired and unrelabeled, and the SE-001-PUB, MI-005-PUB, and MI-006-R1-PUB publication records are unchanged. MI-007 execution, its Nestfolio delta publication, sustained dogfooding, and broader work remain blocked pending explicit authorization.**

**On 2026-07-18, under DR-0024, MI-007 — Governed Learning and Selective Current-Runtime Retirement was executed from the published contract revision `29b592c04162507b9495737a3214149a03de3cee` and closed UNCONDITIONAL PASS — the program's first governed Decision-and-Learning slice, executed through the pinned store API with the engine unmodified (`recordLesson` never invoked), with the human program owner (fabio.vitali) available in-session for the live review capture.** Every precondition verified fail-closed before mutation (37 checks): session gates (three repositories clean on main; Nestfolio HEAD `6229bb010d76723aaec0385c923b157762ee512e`), the engine-lock (20 `runtime/continuity/**` files plus `.claude/settings.json`, aggregate `7e31ff56…`), every bound learning-source digest (the three run-mi006-r1 Evidence artifacts, the classified Guard `74008f12…`, LESSONS.md `eea79ec7…`), the Level 2/3/4 digests, the full byte-identity set, the read-only route facts (run-mi006-r1 completed, run-mi005 closed, no active lease; `.git/journal` tree digest `38595c19…`, byte-identical at close), and the fixed-path absences; the baseline suites returned Level 6 10/10, Level 5 7/7, Level 4 23/23, Level 3 23/23, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68. The complete write-set was first rehearsed in an isolated copy outside all three repositories (placeholder `REHEARSAL-PENDING-OWNER-CAPTURE` in all authorization fields; learning suite 11/11; byte-exact rollback; copy removed with absence proof), freezing 13 created + 2 modified paths. In the real tree, through the pinned store API only: five Observations were created (rev 1) with exact provenance; the four factual ones were advanced (rev 2) to the owner's live verbatim outcomes — `accepted`/`duplicate`/`deferred`/`rejected` ("Confermo: 01 accepted, 02 duplicate, 03 deferred, 04 rejected", machine-captured UTC `2026-07-18T16:56:45.000Z`) — while the fifth, the F-30 historical import, stayed at rev 1 status `historical`, never promoted; `lesson-mi007-01` was created candidate and advanced to accepted ("confermo", `2026-07-18T16:58:57.000Z`) and `lesson-mi007-02` was created rejected-unsafe, both `promotion.automatic false`; `decision-mi007-01` recorded the owner's durable Decision ("ok", `2026-07-18T17:00:48.000Z`); `change-proposal-mi007-01` advanced proposed→accepted→applied; and exactly ONE owner-applied additive binding record (`continuity/bindings/nestfolio/decisions/mi007-deterministic-evidence-boundary-rule.json`) was created only after the recorded Decision and acceptance ("ok", `2026-07-18T17:01:57.000Z`), with the ordering proven by the audit ledger sequence (decision-create and change-proposal-accept strictly precede applied-change-create) and the machine-captured UTCs (Decision precedes application). The no-auto-promotion proof passed over every Skill/Pack/engine/suite/binding/store/lock digest set; the retirement inventory classifies exactly the six migration-plan backward-edge mechanisms (retained/deferred/rejected for target-routed use, each with its no-active-dependency basis) and retires nothing; every mandatory failure was demonstrated (ten fixture-based with exact typed diagnostics plus eight standing prohibitions verified structurally); the audit ledger's pre-execution bytes remain the exact prefix; the derived index rebuilt deterministically discovering the new `decisions` and `change-proposals` kinds; the journal tree is byte-identical. All required suites passed post-execution (learning 11/11, Level 6 10/10, Level 5 7/7, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68 in the real tree; Level 4 23/23 and Level 3 23/23 in an isolated committed validation copy per the SE-001-R3/MI-005 mechanics, removed with absence proof); every isolated fixture and the rehearsal copy were removed with absence proofs; and the Nestfolio execution delta is exactly the fixed create paths plus the two permitted modified paths — 2 modified, 34 created, nothing staged, uncommitted and unstaged (manifest self-hash `53c29538ee56b2b620dcb9ec48509feacee29e62d76351578c8cca7a33878c19`); its publication requires separate explicit authorization. The pinned engine and every published record — including every run-mi006-r1 and VS-001 learning artifact — remain byte-identical. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, and MI-006-R1 PASS remain twelve separate immutable results, and MI-007 UNCONDITIONAL PASS is a separate new immutable result (the thirteenth); the SE-001-PUB, MI-005-PUB, and MI-006-R1-PUB publication records are unchanged. Report: `docs/70-implementation/mi-007-governed-learning-and-selective-current-runtime-retirement.md`. Per the migration plan, sustained dogfooding against the Product Foundation success criteria becomes the next contractable program phase; the MI-007 Nestfolio delta publication (MI-007-PUB), sustained dogfooding, and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, the MI-007 Nestfolio execution-delta publication was executed: session gates verified continuity-workspace and continuity-lab clean on main (continuity-lab HEAD `d658441f6fd500e386dc693782129ac44e6da231`) and Nestfolio HEAD `6229bb010d76723aaec0385c923b157762ee512e` carrying exactly the recorded 36-path delta unstaged and byte-exact against the recorded SHA-256 manifest (manifest self-hash `53c29538ee56b2b620dcb9ec48509feacee29e62d76351578c8cca7a33878c19` recomputed and verified); the 36 paths were staged and committed as one commit, exact subject "Publish MI-007 governed learning and selective current-runtime retirement evidence", sole parent `6229bb010d76723aaec0385c923b157762ee512e`, published at exact Nestfolio revision `914456ce44c271d5bb38b22d985448011d6adcf9`, and pushed to origin/main.** A pre-existing repository ship-gate (`backlog-index-matches`: `docs/BACKLOG.md` stale, unrelated to the MI-007 delta which touches no `docs/backlog/**` path) was resolved through the repository's own journaled `RUNTIME_GATE_SKIP` mechanism on the first attempt; the skip is journaled for ship-recheck adjudication and did not alter the 36-path manifest or its self-hash. At the published revision, Nestfolio is clean and the learning suite, Level 6, Level 5, Level 4, Level 3, Level 2, Level 1, engine, and backlog-next return 11/11, 10/10, 7/7, 23/23, 23/23, 23/23, 14/14, 9/9, and 68/68 respectively, all directly in the real working tree once clean. This publication records repository state only: no completion, Run, Assurance, or Level 6 authority beyond what MI-007 already recorded, and every prior immutable result and the MI-007 execution record remain unchanged. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, MI-006-R1 PASS, and MI-007 PASS remain thirteen separate immutable results, unrepaired and unrelabeled. Per the migration plan, sustained dogfooding against the Product Foundation success criteria becomes the next contractable program phase; sustained dogfooding and broader work remain blocked pending separate contracting, review, publication, and explicit authorization.**

**On 2026-07-18, under DR-0024, the SD-001 — Sustained Dogfooding Bootstrap contract was authored, reviewed by the human program owner, and published on continuity-lab main.** Sustained dogfooding is not a numbered migration slice with a pre-drafted template: it is the validation period against the twelve Product Foundation success criteria and the minimum dogfooding period (six consecutive weeks of active development, twenty non-trivial Work Items, five multi-item or multi-session efforts including two Epics or equivalent, fifteen post-interruption resumptions), bound by digest to `docs/10-product/product-foundation.md` section "Sustained Nestfolio dogfooding success criteria — Provisional Validation Contract" (SHA-256 `223df2894f1b265ea46d16ce9a6031d48d15078ce391cc10db8dab385563f3ab`). SD-001 is the minimal bounded first contract that opens the period under governance. The owner approved the recommended design in the authoring session: a frozen measurement protocol plus an append-only dogfooding ledger under `continuity/evidence/sd-001/` (contemporaneous sampling because criteria 1, 3, 4, and 9 cannot be reconstructed retroactively; weekly cadence; every count derived from existing store artifacts — Work Items, Runs, Checkpoints, Sessions, Evidence, Lessons, Guards — with no new product mechanism ahead of evidence), the SD-001 naming with queue position 35, and authoring plus publication in one session. Key contract properties: the bootstrap execution performs NO store write (exactly two created evidence files, nothing else); the pinned engine stays frozen for the whole period (a defect surfaced by real use routes through the MI-007 governed-learning path plus a separately contracted bounded slice); the period begins at the SD-001-PUB publication commit's committer UTC, from which the standing period rules authorize backlog-routed Nestfolio product work through the Continuity Level 1-6 mechanisms with routine ledger appends shipping through Nestfolio's normal mechanisms (no per-append program ceremony); and the period verdict (working name SD-002) is a separately contracted later human adjudication that SD-001 never issues or anticipates. Execution is NOT authorized by the authoring session: it requires either a later fresh session launched under `sessions/SD-001-sustained-dogfooding-bootstrap/prompt.md` with the exact published contract revision resolved and the human program owner available in-session for the live period-start confirmation, or an explicit same-session extension after publication. DR-0025 closure was performed (queue_position 35, queue_summary, indices regenerated, `--check` exit 0). MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, MI-006-R1 PASS, and MI-007 PASS remain thirteen separate immutable results, unrepaired and unrelabeled, and the SE-001-PUB, MI-005-PUB, MI-006-R1-PUB, and MI-007-PUB publication records are unchanged. SD-001 execution, SD-001-PUB, the dogfooding period, its verdict, and broader work remain blocked pending explicit authorization.**

**On 2026-07-18, under DR-0024, SD-001 — Sustained Dogfooding Bootstrap was executed from the published contract revision `0585f8a576f914b3edfe2518e294730d20ccb87c` and closed UNCONDITIONAL PASS — the first bounded contract opening the sustained-dogfooding validation period under governance, with the human program owner (fabio.vitali) available in-session for the live period-start confirmation.** Every precondition verified fail-closed before mutation: session gates (three repositories clean on main; Nestfolio HEAD `914456ce44c271d5bb38b22d985448011d6adcf9`), the engine-lock (20 `runtime/continuity/**` files plus `.claude/settings.json`, aggregate `7e31ff56…`), the bound criteria-source whole-file digest (`docs/10-product/product-foundation.md` SHA-256 `223df289…`, section title verified), the pre-execution audit-ledger/derived-index/journal-tree digests, the fixed-path absence, and the baseline suites (learning 11/11, Level 6 10/10, Level 5 7/7, Level 4 23/23, Level 3 23/23, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68). `continuity/evidence/sd-001/dogfooding-protocol.md` was materialized with exactly the contracted definitions, twelve criterion derivations, cadence, and frozen-protocol/amendment rules (SHA-256 `95f7f45e…`); the period-start statement was presented live, and the owner replied verbatim **"vai"** (machine-captured UTC `2026-07-18T19:22:53.000Z`) — an affirmative go-ahead, not silence, refusal, or contradiction; `continuity/evidence/sd-001/dogfooding-ledger.md` was created with entry 0 exactly per `entry_zero`, explicitly recording that the period has NOT yet begun (publication is separate) and the zeroed counters (0/20 Work Items, 0/5 multi-item efforts with 0/2 Epics, 0/15 resumptions, 0/6 active weeks). F-PROTOCOL-DRIFT and F-LEDGER-REWRITE were demonstrated in isolated fixtures outside all three repositories with exact digests and cleanup proof; the remaining seven mandatory failures were verified structurally. Post-execution, the audit ledger, derived index, and journal tree digests are unchanged and every pre-existing tracked Nestfolio path equals the exact start revision; all required suites pass (learning 11/11, Level 6 10/10, Level 5 7/7, Level 2 23/23, Level 1 14/14, engine 9/9, backlog-next 68/68 in the real tree; Level 4 23/23 and Level 3 23/23 in an isolated committed validation copy per the SE-001-R3/MI-005 mechanics — a throwaway commit that never entered any real repository history, removed with absence proof). The final Nestfolio manifest is exactly the two fixed create paths, uncommitted and unstaged (SHA-256 manifest self-hash `da770f29…`). SD-001 performed NO store write: neither `continuity/artifacts/**` nor `.continuity/**` changed by one byte, and the pinned engine remains frozen and byte-identical. SD-001 UNCONDITIONAL PASS is a separate new immutable result (the fourteenth); MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, MI-006-R1 PASS, and MI-007 PASS remain thirteen separate immutable results, unrepaired and unrelabeled, and the SE-001-PUB, MI-005-PUB, MI-006-R1-PUB, and MI-007-PUB publication records are unchanged. Report: `docs/70-implementation/sd-001-sustained-dogfooding-bootstrap.md`. SD-001-PUB (publishing the uncommitted 2-path delta, starting the dogfooding period at its committer UTC) requires separate explicit authorization, which the owner may grant as a same-session extension; the period verdict (working name SD-002) and broader work remain blocked.**

**On 2026-07-18, under DR-0024, the owner granted SD-001-PUB as a same-session extension: the recorded 2-path delta (manifest self-hash `da770f29f65535bef51b916e6ef9c9f69c7cd150fefd0477454036dba9e585af`) was verified byte-exact against Nestfolio HEAD `914456ce44c271d5bb38b22d985448011d6adcf9`, staged, and committed as one commit, exact subject "Start SD-001 sustained dogfooding period", published at exact Nestfolio revision `bd0b2fcc2d56fecbcc45c05c80cfe135605b44d3`, and pushed to `origin/main`.** A pre-existing repository ship-gate (`backlog-index-matches`: `docs/BACKLOG.md` stale, unrelated to the SD-001 delta which touches no `docs/backlog/**` path) was resolved through the repository's own journaled `RUNTIME_GATE_SKIP` mechanism on the first attempt, as for MI-007-PUB; the skip is journaled for ship-recheck adjudication and did not alter the 2-path manifest or its self-hash. At the published revision Nestfolio is clean and the learning suite, Level 6, Level 5, Level 4, Level 3, Level 2, and Level 1 return 11/11, 10/10, 7/7, 23/23, 23/23, 23/23, and 14/14 respectively, all directly in the real working tree once clean. Publication records repository state only: no completion, Run, Assurance, or Level 6 authority beyond what SD-001 already recorded. **The sustained-dogfooding period begins at this commit's committer UTC `2026-07-18T21:39:42.000Z`.** From this moment the standing period rules (SD-001 contract, `standing_period_rules`) govern real backlog-routed Nestfolio work through the Continuity Level 1-6 mechanisms, with the pinned engine frozen for the whole period and contemporaneous ledger measurement toward the twelve Product Foundation success criteria. MI-002-R2 PASS, MI-003 FAIL, MI-003-R1 FAIL, MI-003-R2 PASS, MI-004 PASS, SE-001-R1 FAIL, MI-004-R1 PASS, SE-001-R2 FAIL, SE-001-R3 PASS, MI-005 PASS, MI-006 FAIL, MI-006-R1 PASS, MI-007 PASS, and SD-001 PASS remain fourteen separate immutable results, unrepaired and unrelabeled. The period verdict (working name SD-002) is a separately contracted later session, contractable at the earliest when the minimum period (six consecutive weeks, twenty non-trivial Work Items, five multi-item efforts including two Epics, fifteen resumptions) is satisfiable, and never earlier; broader and unrelated work remain blocked.**
...
32. MI-006 — Evidence-Bound Completion and Guard Coexistence — executed 2026-07-18 under DR-0024; UNCONDITIONAL FAIL (ENGINE_POLICY_MISMATCH) fail-closed before any Nestfolio mutation: the pinned engine cannot form the effort's Level 6 Assurance Plan or Evidence-bound complete it without violating the write boundary, because the effort work-item id dashboard-bff-awaiting-confirmation-activity-gap is occupied by an immutable in_progress MI-005 record (rev 2) and selectWork returns REVISION_CONFLICT; Nestfolio unchanged at eafa09b8; ten prior immutable results and SE-001-PUB/MI-005-PUB unchanged; MI-007 blocked
33. MI-006-R1 — Evidence-Bound Completion Existing-Work-Item Rebinding Correction — executed 2026-07-18 under DR-0024; UNCONDITIONAL PASS: the program's first Evidence-bound Work completion, through the pinned engine unmodified, advancing the existing in_progress Work Item (rev 2 -> 3 rebinding write expectedRevision 2 -> 4 startRun -> 5 completeRun) under one bounded immutability exception bound to the three Level 3 criteria, with formation of scope-mi006-r1-completion and ws-mi006-r1, three linked immutable Evidence artifacts spanning deterministic/agent-review/human-review modes, the classified dead-handler Guard evaluated true, the truthful backlog completion write-back (status shipped, b656733 -> ea565c07), and the bounded four-assertion completion-aware suite correction; every mandatory failure fail-closed with the exact diagnostic; Nestfolio 52-path delta uncommitted (6 modified, 46 created; manifest 840c12c0); run-mi005 byte-identical and now permanently non-resumable by staleness (accepted, documented); twelve immutable results and SE-001-PUB/MI-005-PUB unchanged; MI-007 blocked
34. MI-007 — Governed Learning and Selective Current-Runtime Retirement — executed 2026-07-18 under DR-0024; UNCONDITIONAL PASS: the program's first governed Decision-and-Learning slice over the completed effort's run-mi006-r1 Evidence, through the pinned store API with the engine unmodified (recordLesson never invoked); five Observations (four reviewed live to accepted/duplicate/deferred/rejected, one F-30 historical import never promoted), two Lessons (accepted candidate; rejected-unsafe), one durable Decision and one separate Change Proposal advanced proposed-to-accepted-to-applied, and exactly ONE owner-applied additive project-binding rule created only after the recorded Decision and acceptance (audit-order and machine-captured UTC proven); no-auto-promotion proof over every Skill/Pack/engine/suite/binding/store/lock digest; six-mechanism retirement inventory retiring nothing; every mandatory failure fail-closed with its exact diagnostic; all suites green (learning 11/11, L6 10/10, L5 7/7, L4 23/23, L3 23/23, L2 23/23, L1 14/14, engine 9/9, backlog-next 68/68); Nestfolio 36-path delta uncommitted (2 modified, 34 created; manifest 53c29538); pinned engine and every run-mi006-r1/VS-001 record byte-identical; thirteen immutable results; MI-007-PUB and sustained dogfooding blocked
35. SD-001 — Sustained Dogfooding Bootstrap — executed 2026-07-18 under DR-0024; UNCONDITIONAL PASS: every precondition verified fail-closed before mutation (session gates, engine-lock aggregate 7e31ff56…, the bound criteria-source whole-file digest 223df289… with section title verified, the pre-execution audit/index/journal digests, the fixed-path absences, and the baseline suites); the frozen measurement protocol was materialized exactly as contracted (digest 95f7f45e…); the owner's live verbatim period-start confirmation (vai) was captured with machine-captured UTC and the append-only ledger was created with entry 0 exactly per entry_zero, explicitly recording the period as NOT yet begun; F-PROTOCOL-DRIFT and F-LEDGER-REWRITE were demonstrated in isolated fixtures with exact digests and cleanup proof, the remaining seven mandatory failures verified structurally; full byte-identity proven (audit ledger, derived index, and journal tree unchanged; every pre-existing tracked path equal to the exact start revision); all required suites pass (learning 11/11, L6 10/10, L5 7/7, L4 23/23 and L3 23/23 in an isolated committed validation copy, L2 23/23, L1 14/14, engine 9/9, backlog-next 68/68); Nestfolio 2-path delta uncommitted and unstaged (manifest self-hash da770f29…); fourteen immutable results now stand; publication (SD-001-PUB) separately authorized
36. broader implementation and unrelated migration — blocked
...
- MI-006 (Evidence-Bound Completion and Guard Coexistence) is complete with UNCONDITIONAL FAIL (2026-07-18), primary diagnostic `ENGINE_POLICY_MISMATCH`, fail-closed before any Nestfolio mutation (Nestfolio unchanged and clean at `eafa09b81a600bd58fe8f4639298a182039d21f5`). The pinned engine cannot form the effort's Level 6 Assurance Plan from a fresh Work Item carrying the three Level 3 criteria, nor Evidence-bound complete it, without violating the write boundary: the engine keys one Work Item per `work_id`, the backlog write-back forces `workItem.id` to be the effort id, and that work-item artifact already exists at revision 2, status `in_progress`, as an immutable MI-005 record (dogfood criterion `mi005-keyed-effect-materialized`) because MI-005 ran its Level 5 dogfood Run on the effort's real work id — so `selectWork(effort)` returns `REVISION_CONFLICT` (proven empirically, isolated copy removed with absence proof) and reusing/advancing it would mutate an immutable MI-005 record and validate the wrong criteria. A separate owner-authorized secondary finding (criterion 2/3 Evidence names the dashboard-bff integration suite, but the awaiting-confirmation projection is unit-tested and the integration suite targets real deployed AWS; owner authorized unit suite plus non-regression, UTC `2026-07-18T11:38:34.000Z`) is recorded but is not the cause. MI-006 UNCONDITIONAL FAIL is a separate new immutable result. The next operation is a newly bounded correction (an engine revision enabling a fresh Level 6 completion path for an effort whose work-item id was already used, or a re-scoped MI-006-R1 advancing the existing in_progress Work Item under a bounded immutability exception and fixing the integration-suite Evidence wording); this correction was contracted, published, and executed as MI-006-R1.
- MI-006-R1 (Evidence-Bound Completion Existing-Work-Item Rebinding Correction) is complete with UNCONDITIONAL PASS (2026-07-18), executed from the published contract revision `80d70ac9bf9f1252474a9fbed414a0ef6ee57061`. It is the program's first Evidence-bound Work completion, executed through the pinned engine unmodified: the effort's existing `in_progress` Work Item advanced from the immutable revision-2 MI-005 base to completed through the three contracted transitions (rev 3 rebinding write with `expectedRevision 2`, rev 4 engine `startRun`, rev 5 engine `completeRun`) under one bounded immutability exception bound to the three Level 3 criteria, with a bounded formation exception (`scope-mi006-r1-completion`, `ws-mi006-r1`), three linked immutable Evidence artifacts spanning deterministic/agent-review/human-review modes, one classified dead-handler Guard evaluated true, and the truthful backlog completion write-back (status shipped; `b656733…` → `ea565c07…`). Every mandatory failure fail-closed with the exact diagnostic; the Waiver path was demonstrated on a synthetic criterion. The four self-invalidating published assertions received the bounded completion-aware correction and pass in both states. The Nestfolio 52-path execution delta (6 modified, 46 created; manifest self-hash `840c12c0ae31e1884a4221372b873be5576bcd7097485d29bcb938e7d0b93fea`) remains uncommitted and unstaged; its publication requires a separate explicit authorization. `run-mi005` is byte-identical and now permanently non-resumable by staleness (accepted, documented). MI-006-R1 UNCONDITIONAL PASS is a separate new immutable result; MI-007 and broader work remain blocked.
...
- MI-006 completed contract (UNCONDITIONAL FAIL): `sessions/MI-006-evidence-bound-completion-and-guard-coexistence/context-pack.yaml`
- MI-006 execution prompt (consumed 2026-07-18): `sessions/MI-006-evidence-bound-completion-and-guard-coexistence/prompt.md`
- MI-006 execution-closure handoff: `sessions/MI-006-evidence-bound-completion-and-guard-coexistence/session-handoff.md`
- MI-006 execution-closure manifest (Nestfolio delta EMPTY; fail-closed before mutation): `sessions/MI-006-evidence-bound-completion-and-guard-coexistence/execution-changed-files.txt`
- MI-006 report: `docs/70-implementation/mi-006-evidence-bound-completion-and-guard-coexistence.md`
- MI-006-R1 completed contract with appended execution closure (UNCONDITIONAL PASS): `sessions/MI-006-R1-evidence-bound-completion-existing-work-item-rebinding/context-pack.yaml`
- MI-006-R1 execution prompt (consumed 2026-07-18): `sessions/MI-006-R1-evidence-bound-completion-existing-work-item-rebinding/prompt.md`
- MI-006-R1 execution-closure handoff: `sessions/MI-006-R1-evidence-bound-completion-existing-work-item-rebinding/session-handoff.md`
- MI-006-R1 execution-closure manifest: `sessions/MI-006-R1-evidence-bound-completion-existing-work-item-rebinding/execution-changed-files.txt`
- MI-006-R1 report: `docs/70-implementation/mi-006-r1-evidence-bound-completion-existing-work-item-rebinding.md`
- MI-006-R1 Nestfolio Level 6 Assurance completion and evidence (UNCOMMITTED at Nestfolio `eafa09b81a600bd58fe8f4639298a182039d21f5`; publication separately authorized): `continuity/level-6/**`, `tests/continuity-level-6.test.mjs`, `continuity/bindings/nestfolio/guards/mi006-r1-dashboard-bff-no-dead-user-confirmation-requested-handler.json`, `continuity/evidence/mi-006-r1/**`, the completed Work Item revision chain, and the frozen `run-mi006-r1` engine write-set; 52-path delta manifest self-hash `840c12c0ae31e1884a4221372b873be5576bcd7097485d29bcb938e7d0b93fea`
