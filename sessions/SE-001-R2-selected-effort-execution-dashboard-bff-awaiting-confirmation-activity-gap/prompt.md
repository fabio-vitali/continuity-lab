---
artifact_id: SESSION-PROMPT-SE-001-R2
status: provisional
owner: session
last_updated: 2026-07-17
supersedes: [SESSION-PROMPT-SE-001-R1]
superseded_by: null
---

# SE-001-R2 — Preserved Future Execution Instruction
...
```text
This is a fresh SE-001-R2 — Selected Effort Execution session. It authorizes
bounded execution of the contract at continuity-lab revision
<EXACT-PUBLISHED-SE-001-R2-CONTRACT-REVISION>: implementing exactly the
...
sessions/SE-001-R2-selected-effort-execution-dashboard-bff-awaiting-confirmation-activity-gap/context-pack.yaml
...
mutation (Level 4 23/23 is satisfiable there — verified by MI-004-R1).
Prove the awaiting-confirmation activity gap: the contracted validation
fails in an isolated temporary copy at the exact start revision and passes
in the implemented working tree. The retained read-only SE-001-R1
investigation at ~/continuity-recovery/se-001-r1/ledger/failure-diagnosis.md
is admissible planning input only (planned sourcing: investor-adpt forwards
...
per decision); every binding is re-verified fail-closed.
...
SE-001-R2). Keep every other pre-existing tracked byte immutable, including
...
the contract's rollback rule and close SE-001-R2 as UNCONDITIONAL FAIL.
...
MI-004 PASS, SE-001-R1 FAIL, and MI-004-R1 PASS as seven separate immutable
results; SE-001 remains published-superseded with no execution result.

Report the SE-001-R2 result, gap proof, frozen boundary, digest
